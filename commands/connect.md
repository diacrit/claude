...
8. Bookmark index setup:
   ...
   - If yes:
     ...
     c. Append this line to the `## Reference Trees` section of `~/.claude/CLAUDE.md` (after the last `- ` bullet in that section):
        ```
        - [`~/.config/diacrit/index.md`](~/.config/diacrit/index.md) — Your saved bookmarks. Consult when the user mentions `diacrit`, bookmarks, a saved link, or a topic that could be a cached page.
        ```
